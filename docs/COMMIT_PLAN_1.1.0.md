...
태그 실행이 끝나면 **Releases**에 릴리즈가 바로 공개되고 Latest 배지가 옮겨 간다.
본문은 설치 표 + `docs/RELEASE_NOTES_<버전>.md` 전문으로 자동으로 채워진다.
승인 단계가 없으므로, 태그를 밀기 전 시험 실행이 마지막 관문이다.
...
이미 공개된 릴리즈는 GitHub 웹에서 먼저 지운다. 안 지우고 태그를 다시 밀면 같은
이름의 릴리즈가 하나 더 생긴다.
