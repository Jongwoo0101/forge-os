...
넷이 끝나면 `릴리즈에 첨부` 잡이 **바로 공개하고 Latest 배지를 이 릴리즈로 옮긴다**
(`draft: false` · `make_latest: true`). 손으로 눌러야 할 것은 없다.

본문도 자동으로 채워진다. 설치 표와 macOS 격리 속성 안내를 위에 붙이고, 그 아래에
`docs/RELEASE_NOTES_<버전>.md` **전문**을 그대로 넣는다. 릴리즈 페이지를 연 사람이
가장 먼저 찾는 것은 "어느 파일을 받아야 하는가"이지 변경 내역이 아니므로 순서가 그렇다.

> **노트 파일 이름이 태그와 맞아야 한다.** 태그 `v2.0.0` → `docs/RELEASE_NOTES_2.0.0.md`.
> 파일이 없으면 릴리즈는 그대로 만들어지되 본문에 경고 한 줄이 대신 들어간다 —
> 노트를 안 썼다고 배포를 통째로 실패시키지는 않는다.

**게시 승인 단계가 없다는 뜻이기도 하다.** 태그를 미는 순간 공개된다. 그래서 5번의
시험 실행(`workflow_dispatch`)이 실질적인 마지막 관문이다. 산출물을 미리 받아
실행해 보고 태그를 밀 것.
...
**릴리즈가 이미 공개된 뒤라면** GitHub 웹에서 그 릴리즈를 먼저 지우고 태그를 지운다.
안 지우고 태그를 다시 밀면 같은 이름의 릴리즈가 하나 더 생긴다. 이미 누가 받아 갔을
수도 있으므로, 공개된 릴리즈를 되돌리는 것보다 **다음 패치 버전을 내는 쪽**이 대개 낫다.
