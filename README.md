...
> **1.1.1에서 다시 만들었습니다.** 이전에는 10초짜리 MP4였습니다. 지금은 도형뿐입니다 —
> 리소스도, 디코더도, `javafx.media` 모듈도 필요하지 않습니다.

콘솔이 흐려지는 **바로 그 자리에서** 로고가 떠오릅니다. 두 장면이 이어 붙는 대신
겹치므로 검은 화면만 보이는 구간이 없습니다.
...
| 요소 | 움직임 |
|---|---|
| **후광** | 점선 원 두 겹(r=268 · 188)이 각각 초당 11° · 17°로 시계 방향. 배경화면의 동심원과 같은 자리라 부팅이 끝나고 배경이 드러날 때 이어져 보입니다 |
| **로고** | `ForgeMark` — 배경화면이 쓰는 것과 **같은** 벡터입니다. 0.55초 동안 커지며 밝아집니다 |
| **스피너** | 초당 210° 시계 방향. 호의 길이가 34° ↔ 292°를 1.5초 주기로 오갑니다 |
| **안쪽 호** | 초당 335°, 청록 실선. 하나만 돌면 시계처럼 보이고, 둘이 다른 속도로 돌아야 기계처럼 보입니다 |
| **불티** | 호의 머리에 붙어 돕니다. 지금 어디까지 왔는지 알려 주는 유일한 점입니다 |
| **상태 문구** | 0.3초 "커널 준비 완료" → 1.0초 "그래픽 계층 초기화" → 1.6초 "데스크탑 구성 중" |

마지막 0.5초에 **호가 완전한 원으로 닫히면서** 전체가 살짝 커집니다. 로딩이 흐지부지
사라지는 대신 "채워졌다"로 끝나야, 다음에 오는 데스크탑이 그 결과로 읽힙니다.

### 3단계 — 데스크탑

스플래시가 제 연출을 끝내면 부드럽게 페이드되며 바탕화면 · 상단 메뉴바 ·
하단 Dock으로 전환됩니다.

> **단계 전환은 시간이 아니라 사건으로 이어집니다.** 고정된 초로 이어 붙이면 반드시
> 어긋납니다 — 커널 부팅 시간은 기계마다 다르고, 타이핑 시간은 로그 길이에 따라
> 달라지기 때문입니다. 1 → 2 는 *커널 부팅 완료 **그리고** 타이핑 큐 소진*이 신호입니다.
> 2 → 3 만은 시간으로 정해집니다 — 스플래시는 기다릴 사건이 없기 때문입니다.
...
└── src/main/
    ...
    │       │   └── BootSplash.java      # 로고 · 스피너 · 후광 (전부 벡터, 타이머 하나)
    │       ├── desktop/
    ...
    │           └── ToggleSwitch.java
    └── resources/
        ├── assets/                      # 로고 · 배너 SVG (부팅 MP4는 1.1.1부터 쓰지 않습니다)
        └── forgeos/css/
            ├── tokens-dark.css          # 색만
            ├── tokens-light.css         # 색만
            ├── theme.css                # 셸 구조·치수
